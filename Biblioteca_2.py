...
class Ventana_principal(Gtk.Window):
    def __init__(self):
        ...
        # Texto en la ventana principal que guia al usuario sobre que hacer.
...
class DialogoSobre(Gtk.AboutDialog):
    def __init__(self):
        super().__init__()
        # Se define la resolucion del dialogo About.
        self.resize(300,300)
...
class DialogoIngresar(Gtk.Dialog):
    def __init__(self, parent):
        ...
        # Se configuran los bordes y la resolucion de la pantalla.
        self.set_border_width(10)
        self.resize(500, 100)
    ...
    def eleccion(self, btn=None):
        ...
        # Si no se selecciona una opcion salta un Warning que dice que eligas una opcion.
        else:
            dialogo_opcion = Gtk.MessageDialog(
            transient_for = self,
            flags = 0,
            message_type = Gtk.MessageType.WARNING,
            buttons = Gtk.ButtonsType.OK_CANCEL ,
            text = "Eliga una opcion por favor" 
            )
            respuesta_ = dialogo_opcion.run()

            if respuesta_ == Gtk.ResponseType.OK:
                dialogo_opcion.destroy()
            elif respuesta_ == Gtk.ResponseType.CANCEL:
                dialogo_opcion.destroy()
        
            dialogo_opcion.destroy()
    
    # Funcion que te hace volver a la ventana principal
    def regresar(self, btn=None):
        self.destroy()

# Dialogo donde se agregan los libros.
class DialogoAgregar(Gtk.Dialog):
    def __init__(self, parent):
        ...
        contenedor_1 = self.get_content_area()
